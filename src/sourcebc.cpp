...
SourceBC::SourceBC(Element *startElement,
      ...
      case HeatSource:
        for(size_t i = 0; i < m_profile.size(); i++)
        {
          if(m_timeSeries->interpolate(dateTime, i, m_dataCursor, value))
          {
            ...
            element->externalHeatFluxes += value * factor;
          }
        }
        break;
      case FlowSource:
        for(size_t i = 0; i < m_profile.size(); i++)
        {
          if(m_timeSeries->interpolate(dateTime, i, m_dataCursor, value))
          {
            ...
            double factor = m_factors[element];
            element->externalHeatFluxes += m_model->m_waterDensity * m_model->m_cp * value * element->temperature.value * factor;
      ...
      case SoluteSource:
        for(size_t i = 0; i < m_profile.size(); i++)
        {
          if(m_timeSeries->interpolate(dateTime, i, m_dataCursor, value))
          {
            Element *element = m_profile[i];
            double factor = m_factors[element];
            element->externalSoluteFluxes[m_soluteIndex] += value * factor;
          }
        }
        break;
    }
